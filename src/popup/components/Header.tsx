...
interface Props {
  ...
  /**
   * Show the round profile-switcher avatar in the leading slot of the
   * actions area. Defaults to true wherever a fingerprint is shown — opt
   * out on the vaults screen itself to avoid a tap that does nothing.
   */
  showVaultAvatar?: boolean;
}

/** Common popup header with the brand glyph + an optional subtitle/actions. */
export function Header({ subtitle, fingerprint, actions, showVaultAvatar = true }: Props) {
  const hasFingerprint = fingerprint !== undefined && fingerprint !== null;
  const showAvatar = showVaultAvatar && hasFingerprint;
  return (
    <header class="flex items-center justify-between gap-3">
      ...
      {showAvatar || actions !== undefined ? (
        <div class="flex items-center gap-1 shrink-0">
          {showAvatar ? <VaultAvatar fingerprint={fingerprint as string} /> : null}
          {actions}
        </div>
      ) : null}
    </header>
